...
def load(path: Union[str, object], spec_version=None, print_out: bool = True) -> Union[
	tuple[list[int, int], str, TILES, GRID, list], None]:
	"""
	...
	def load_v0_12():
		...
		tiles: dict[str, PureTileGroup] = {
			'all': PureTileGroup('all', {tile: tuple(map(int, pos.lstrip('(').rstrip(')').split(',')))
...
class Main:
	...
	def refresh(self):
		...
		main_project = self.Header.render('< ' + (
			selected_project.path.split('/')[-1] if selected_project is not None else '') + ' > ', True,
...
class Project:
	...
	def save(self):
		...
		if self.path is not None and self.path[-4:] != '.png':
			save_data = json.dumps(
				{
					'grid':          [{f"{pos[0]},{pos[1]}": tile for pos, tile in layer.items()} for layer in
									  self.grid],
							...
							if name in tile_group:
								...
								if vis_rect.collidepoint(x, y):
									if tile_name not in self.tile_cache:
										self.tile_cache[
											tile_name] = pg.transform.scale(self.sprite_sheet.img.subsurface(tile_name), size).convert_alpha()
	...
	def render(self):
		...
		if self.tile_mode_enabled:
			...
			# ===[ BOLD LINES ]===
			pg.draw.line(self.display, "white", (self.bold.x, self.main.Options.TOP_OFFSET), (
			self.bold.x, dis_rect.h), 5)
			...
			if event.type == KEYDOWN:
				...
				if not self.tile_mode_enabled:
					if self.tool == 'rect' or self.tool == 'autotile-rect':
						if self.rect[0]:
							...
							width = (
								rect.left if rect.w > 0 else rect.right - 1,
								rect.right if rect.w > 0 else rect.left + 1)
							height = (
								rect.top if rect.h > 0 else rect.bottom - 1,
								rect.bottom if rect.h > 0 else rect.top + 1)
							if self.tool == 'rect':
				...
				if event.pos[0] > self.sidebar.right:
					if event.buttons[1]:
						...
						if self.tool == 'brush':
							if not any(group.collidepoint(event.pos) for group in
									   self.tiles.values()) and not self.layers_vis.collidepoint(event.pos):
								self.set_block(self.current_block(event.pos))
						elif self.tool == 'rect' or self.tool == 'autotile-rect':
							if self.rect[0]:
								if not any(group.collidepoint(event.pos) for group in self.tiles.values()):
		...
		for event in events:
			if event.type == MOUSEMOTION:
				...
				if event.button == 1 or event.button == 3 and self.collidepoint(event.pos):
					...
					if pg.Rect(
							self.pos[0] + width - 64,
							self.pos[1] + self.project.scroll + 20, 32, 32).collidepoint(event.pos):
						...
						for idx, (name, tile) in enumerate(self.tiles.items()):
							pos = pg.Vector2(idx % tiles_in_row * 69 + 5, idx // tiles_in_row * 69 + 5 + 64) + (
							self.pos[0], self.pos[1] + self.project.scroll)
...
class Popup:
	...
	def __init__(self, main, display):
			...
			[
				{
					...
					'options':
								{
									option: self.text.render(option, True, (200, 200, 200)) for option in
			...
			[
				{
					...
					'options':
								{
									option: self.text_hover.render(option, True, (200, 200, 200)) for option in
									option_group['options']
								}
				} for option_group in self.DATA
			]
		...
		self.pos = pg.Vector2((
										  self.display.get_width() - self.question.get_width()) / 2 - 100, self.display.get_height() / 3)
			...
			if event.type == MOUSEBUTTONUP:
				if event.button == 1:
					...
					if pg.Rect(rect.x + 25, rect.bottom - 55, rect.w - 50, 50).collidepoint(event.pos) and \
						...
						try:
							self.main.projects.append(Project(self.main, list(map(int,
								self.answer['Tile Size'].split('x')))))
							self.main.selected = len(self.main.projects) - 1
