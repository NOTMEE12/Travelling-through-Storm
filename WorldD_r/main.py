...
def load(
    path: Union[str, object], spec_version=None, print_out: bool = True
) -> Union[tuple[list[int, int], str, TILES, GRID, list], None]:
    """
    ...
    def load_v0_12():
        ...
        tiles: dict[str, PureTileGroup] = {
            "all": PureTileGroup(
                "all", {tile: tuple(map(int, pos.lstrip("(").rstrip(")").split(","))) for pos, tile in data["data"].items()}
            )
        }
        grid: GRID = [{tuple(map(int, pos.split(","))): list(tile) for pos, tile in data["grid"].items()}]
    ...
    def load_v1_00():
        ...
        grid: GRID = [
            {tuple(map(int, map(float, pos.split(",")))): list(tile) for pos, tile in layer.items()} for layer in data["grid"]
        ]
...
class Main:
    ...
    def refresh(self):
        ...
        """====[ PROJECT NAME ]===="""
        pg.draw.rect(
            self.display, self.colors.Welcome["top-bar-background"], (0, 0, self.display.get_width(), self.Options.TOP_OFFSET)
        )
        selected_project = self.projects[self.selected]
        main_project = self.Header.render(
            "< " + (selected_project.path.split("/")[-1] if selected_project is not None else "") + " > ",
            True,
            self.colors.Welcome["top-bar-text-color"],
        )
        main_pos = (
            (self.display.get_width() - main_project.get_width()) / 2,
        ...
        if self.selected > 0:
            for en, project in enumerate(self.projects[: self.selected]):
                ...
                pos = (
                    main_pos[0]
                    - sum(self.SmallerHeader.size(" " + p.path.split("/")[-1] + " ")[0] for p in self.projects[: en + 1]),
        ...
        if self.selected < len(self.projects) - 1:
            for en, project in enumerate(self.projects[self.selected + 1 :]):
                ...
                pos = (
                    main_pos[0]
                    + main_project.get_width()
                    + sum(
                        self.SmallerHeader.size(" " + p.path.split("/")[-1] + " ")[0]
                        for p in self.projects[self.selected + 1 : en - 1]
                    ),
                    (self.Options.TOP_OFFSET - name.get_height()) / 2,
                )
                self.display.blit(name, pos)
        ...
        if self.Options.SHOW_EXIT:
            pg.draw.rect(
                self.display, (180, 180, 180), pg.Rect(self.display.get_width() - 25, 0, 25, 25), border_radius=15, width=5
            )
...
class Project:
    def __init__(self, main: Main, tile_size, load: str | bool = False):
        ...
        self.save_selection_group_sel = self.header.render("group (name/ESC)", True, (250, 250, 250))
        self.save_selection_name_sel = self.header.render("name (name/ESC)", True, (250, 250, 250))
        ...
        self.bold = pg.Vector2(
            self.offset[0] * self.zoom - self.tile_size[0] + self.sidebar.right, self.offset[1] * self.zoom - self.tile_size[1]
        )
        self.tile_cache = {}
    ...
    def save(self):
        if self.destination is None:
            if self.path is None or self.path[-4:] == ".png":
                self.destination = filedialog.asksaveasfile("a+", defaultextension=".world", title="Save world as: ")
    ...
    def display_hover_tile(self, pos=None, tile=None):
        if self.selected_tile is not None or tile is not None:
            ...
            if pg.Rect(0, self.main.Options.TOP_OFFSET, dis_rect.w, dis_rect.h - self.main.Options.TOP_OFFSET).colliderect(
                pg.Rect((x, y), size)
            ):
                if tile is None:
    ...
    def draw_hover_autotiling_rect(self):
        ...
        txt = {key: pg.transform.scale(self.sprite_sheet.img.subsurface(group[value]), size) for key, value in matrix.items()}
        # DRAW TOP
        width = right - left
        height = bottom - top
        ...
        if height > 1:
            self.display_hover_tile((self.bold.x + size[0] * left, self.bold.y + size[1] * bottom - size[1]), txt[(-1, 1)])
            self.display_hover_tile(
                (self.bold.x + size[0] * right - size[0], self.bold.y + size[1] * bottom - size[1]), txt[(1, 1)]
            )
    ...
    def draw_grid_tiles(self):
        size = (self.tile_size.x * self.zoom, self.tile_size.y * self.zoom)
        vis_rect = pg.Rect(
            self.sidebar.right - size[0] + 1,
            self.main.Options.TOP_OFFSET - size[1],
            self.display.get_width() - self.sidebar.w + size[0] * 2,
            self.sidebar.h + size[1],
        )
        grid = []
                            ...
                            if name in tile_group:
                                ...
                                if vis_rect.collidepoint(x, y):
                                    if tile_name not in self.tile_cache:
                                        self.tile_cache[tile_name] = pg.transform.scale(
                                            self.sprite_sheet.img.subsurface(tile_name), size
                                        ).convert_alpha()
            ...
            if event.type == KEYDOWN:
                if self.renaming:
                    ...
                    if event == self.main.Bindings.SCALE_TILE_UP:
                        self.tile_size *= 2
                        self.bold = pg.Vector2(
                            self.offset[0] * self.zoom - self.tile_size[0] + self.sidebar.right,
                ...
                if event.pos[0] > self.sidebar.right:
                    if event.buttons[1]:
                        ...
                        if self.tool == "brush":
                            if not any(
                                group.collidepoint(event.pos) for group in self.tiles.values()
                            ) and not self.layers_vis.collidepoint(event.pos):
                                self.set_block(self.current_block(event.pos))
                        elif self.tool == "rect" or self.tool == "autotile-rect":
                            if self.rect[0]:
                                if not any(group.collidepoint(event.pos) for group in self.tiles.values()):
    ...
    class SpriteSheet:
        def __init__(self, path, display, project):
            ...
            self.center = self.display.get_width() * 0.65, self.display.get_height() / 2
        ...
        def draw_selection(self, img):
            if self.selection != (0, 0, 0, 0):
                draw_rect(
                    ...
                    pg.Rect(
                        self.selection.x * self.zoom,
                        self.selection.y * self.zoom,
                        self.selection.w * self.zoom,
                        self.selection.h * self.zoom,
                    ),
                    width=3,
                )
        ...
        def render(self, tile_size):
            ...
            area = self.img.get_rect(
                topleft=(rect.w / 2 * (self.zoom - 1) + self.offset.x, rect.h / 2 * (self.zoom - 1) + self.offset.y),
                size=(self.w, self.h),
            )
                                        ...
                                        if y % self.project.tile_size[1] == 0:
                                            ...
                                            if self.selection_group_name not in self.project.tiles:
                                                self.project.tiles[self.selection_group_name]: TileGroup = TileGroup(
                                                    self.project, self.selection_group_name, {}
                                                )
                                            self.project.tiles[self.selection_group_name][id_ + f" - {x}x{y}"] = (
                                                x,
                                                y,
...
class TileGroup(PureTileGroup):
    ...
    @property
    def size(self):
        tile_size = (max(64, int(self.project.tile_size[0])), max(64, int(self.project.tile_size[1])))
    ...
    def draw(self):
        ...
        for idx, (name, tile) in enumerate(self.tiles.items()):
            ...
            if (self.name, name) == self.project.raw_selected_tile:
                pg.draw.rect(
                    tiles, self.project.selected_tile_color, pg.Rect(pos.x - 4, pos.y - 4, tile_size[0] + 8, tile_size[1] + 8)
                )
    ...
    def collidepoint(self, *pos):
        ...
        matrix = pg.Rect((pos2.x + width, self.pos.y), (max(256, 3 * tile_size[0]), max(256, 3 * tile_size[1]) + 64))
                ...
                if event.button == 1 or event.button == 3 and self.collidepoint(event.pos):
                    ...
                    if pg.Rect(self.pos[0] + width - 64, self.pos[1] + self.project.scroll + 20, 32, 32).collidepoint(event.pos):
                        ...
                        for idx, (name, tile) in enumerate(self.tiles.items()):
                            pos = pg.Vector2(idx % tiles_in_row * 69 + 5, idx // tiles_in_row * 69 + 5 + 64) + (
                                self.pos[0],
                                self.pos[1] + self.project.scroll,
                            )
    ...
    def data(self):
        return {
            "tiles": {name: tile for name, tile in self.tiles.items()},
            "pos": list(self.pos),
            "_draw_matrix": self._draw_matrix,
            "_matrix": {},
        }
...
class Welcome:
    ...
    def render(self):
        ...
        pg.draw.rect(
            self.display,
            self.main.colors.Welcome["recent-background"],
            rect,
...
class Popup:
    def __init__(self, main, display):
        ...
        self.DATA = [
            {"question": "Type", "options": ["! isometric (no support for now) !", "2(.5)D"]},
            {"question": "Tile Size", "options": ["8x8", "16x16", "24x24", "32x32", "48x48", "64x64"]},
        ]
        self.option_groups = [
            {
                "question": self.small_header.render(option_group["question"], True, (200, 200, 200)),
                "options": {option: self.text.render(option, True, (200, 200, 200)) for option in option_group["options"]},
            }
        ...
        self.option_groups_hover = [
            {
                ...
                "options": {option: self.text_hover.render(option, True, (200, 200, 200)) for option in option_group["options"]},
            }
            for option_group in self.DATA
        ]
                    ...
                    for enum, option_group in enumerate(self.option_groups):
                        ...
                        for en, (name, texture) in enumerate(options.items()):
                            ...
                            if pos.collidepoint(event.pos):
                                ...
                                break
                            top = top + 10 + texture.get_height()
                    if pg.Rect(rect.x + 25, rect.bottom - 55, rect.w - 50, 50).collidepoint(event.pos) and all(
                        self.answer[key] is not None for key in self.answer.keys()
                    ):
                        try:
