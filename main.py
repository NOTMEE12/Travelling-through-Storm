...
class World:
	...
	def block_render_pos(self, grid_pos,
	                     offset: typing.Union[
...
class Player:
	...
	def __init__(self, main):
		...
		self.images = (
			main.sp_sheet.subsurface(16, 32, 16, 16),
			main.sp_sheet.subsurface(16, 16, 16, 16)
		)
...
class Main:
	...
	def __init__(self):
		...
		self.cur_world_id = 0
		self.worlds = (
			CustomWorld(self, 'worlds/1.world',
				[
					Shark(self, (0, 2)),
					Shark(self, (-1, 3))
				]
			...
			CustomWorld(self, 'worlds/9.world', [
				Shark(self, (-2, 4), (-1, 1)),
				Shark(self, (-3, 4), (0, -1)),
		...
		self.sea_sound = pygame.mixer.Sound('sounds/sea.mp3')
	...
	def render(self):
		...
		if self.current_world is not None and self.death_screen.alpha <= 1 and self.too_much_moves.alpha <= 1:
			if not self.sea_sound_channel.get_busy():
			...
			text = f'Moves: {self.current_world.move_count} [/{self.current_world.max_move_count}]\n' \
			...
			self.sea_sound_channel.fadeout(1000)
	...
	def update(self):
		if self.current_world is not None and tuple(self.player.grid_pos) in self.current_world.grid[0] and \
				self.current_world.grid[0][tuple(self.player.grid_pos)][1] == 'end':
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if event.key in (pygame.K_LEFT, pygame.K_UP, pygame.K_DOWN, pygame.K_RIGHT):
					if not self.player.ready_for_next_move or self.current_world is None or self.player.health <= 0 or \
							self.too_much_moves.alpha >= 1:
						continue
					move = {
						pygame.K_LEFT: (-1, 1),
						pygame.K_RIGHT: (1, -1),
	...
	def run(self):
		while True:
			...
			self.window.title = f'{self.window.title.split("|")[0]}| {self.clock.get_fps()}'
